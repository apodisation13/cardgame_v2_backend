...
class UserProgressService:
    ...
    async def get_user_progress(
        ...
        async with self.db_pool.connection() as connection:
            ...
            enemies, enemy_leaders, seasons = await self._process_enemies(
                connection=connection,
                user_id=user_id,
                base_url=base_url,
            )

            user_cards, user_leaders, user_decks = await self._process_cards(
                connection=connection,
                user_id=user_id,
                base_url=base_url,
            )
    ...
    async def _process_enemies(
        ...
        enemies: list[Enemy] = await self._get_enemies(
            connection=connection,
            base_url=base_url,
        )
        ...
        enemy_leaders: list[EnemyLeader] = await self._get_enemy_leaders(
            connection=connection,
            base_url=base_url,
        )
        ...
        seasons: list = await self._get_seasons(
            connection=connection,
            user_id=user_id,
        )
        ...
        user_seasons: list[Season] = await self._construct_seasons(
            seasons=seasons,
    ...
    async def _get_level_related_levels(
        self,
        connection: asyncpg.Connection,
        level_ids: set[int],
    ...
    async def _process_cards(
        ...
        user_cards: list[UserCard] = await self._get_user_cards(
            connection=connection,
            user_id=user_id,
            base_url=base_url,
        )
        ...
        user_leaders: list[UserLeader] = await self._get_user_leaders(
            connection=connection,
            user_id=user_id,
            base_url=base_url,
        )
        ...
        user_decks: list[UserDeck] = await self._contruct_user_decks(
            connection=connection,
            user_id=user_id,
    ...
    async def _contruct_user_decks(
        self,
        connection: asyncpg.Connection,
        user_id: int,
